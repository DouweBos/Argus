...
vi.mock("../../../stores/workspaceStore", () => ({
  useWorkspaceStore: (selector: (s: unknown) => unknown) =>
    mockUseWorkspaceStore(selector),
}));
...
function makeHunk(adds: number, removes: number): DiffHunk {
  ...
  for (let i = 0; i < adds; i++) lines.push(makeLine("add"));
  for (let i = 0; i < removes; i++) lines.push(makeLine("remove"));
  return {
    header: "@@ -1,1 +1,1 @@",
    lines,
    oldStart: 1,
...
function setupDefaults(
  overrides: {
    ...
    workspace?: null | Record<string, unknown>;
  } = {},
) {
  ...
  mockUseWorkspaceStore.mockImplementation(
    (selector: (s: unknown) => unknown) =>
      selector({
        workspaces: workspace ? [{ id: "ws-1", ...workspace }] : [],
      }),
  );
}
